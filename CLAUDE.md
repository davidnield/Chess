...
`rust/explorer_extract/` is the Rust explorer extract (`explorer-extract.exe`); its README holds the
exactness contract and where each part of it is tested. Its `tests/fixtures/` and `src/pydigits.rs`
are GENERATED by `python/gen_rust_fixtures.py` from python-chess and `extract_file` — regenerate
them after changing the Python side of the contract, never hand-edit them.
...
There is no lint config, and no build step except the Rust extract below. Every
`python/_test_*.py` is a standalone pass/fail script (exit 0/1); `run_tests.py` subprocess-runs all
of them with a summary. Convention: synthetic tests import the production query/logic rather than
copying it (`_test_winpos.py` is the template).

### The explorer book's three producers

A finished explorer month is `<out>/month=Y_M/bkt=i/part-0000.parquet` (PS_COLS, plain `string`,
512 buckets by `((parent_hash % 512) + 512) % 512`), `_manifest/month=Y_M.parquet` (26 fields, in
`bucket_month.MANIFEST_FIELDS` order) and the `_month=Y_M.DONE` sentinel written LAST, plus a term
monthly. The A2 merge reads any producer's month without knowing which made it:

| producer | how |
|---|---|
| backfill | extract (EPD to ply 16) → consolidate → `backfill_epd.py` (replays the missing EPDs) |
| B1 | extract with `--epd-max-ply 30` → consolidate → `bucket_month.py` (no replay) |
| Rust | `explorer-extract month` (games → finished month in one tool) |

- **One producer per book.** The backfill quarantines a 64-bit collision twin's rows; B1 and Rust
  keep every row with its true EPD (the A2 merge resolves by EPD). Mixing producers in one book
  would mix those two policies.
- **A partial dir is locked to its settings.** `_extract_params.json` (epd_max_ply, max_ply,
  chunk_games, events, min_elo, exclude_bots, excluded_terminations, producer) is written on first
  use by both extracts and any mismatch is refused: the sentinel-gated resume would otherwise mix
  epd-16 and epd-30 partials, or Python and Rust ones, in one dir.
- **Rust `partials` is exact against the Python extract, `ply` included.** Rust `month` takes MIN
  over the chunks' first plies (never above any `any_value` consolidation could pick, and exactly
  derivable from the partials), MIN EPD and MIN child: commutative, so its output is byte-identical
  across thread and pass counts. `ply1_games` legitimately differs between producers (the start
  position recurs) — never gate on it.
...
- **Build and check the Rust tool:** `RUSTUP_TOOLCHAIN=stable` (this machine's stable is the pinned
  ...
  before real use; the binary is x86-64-v3 and says so if the CPU is older.
