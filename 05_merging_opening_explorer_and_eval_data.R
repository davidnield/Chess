...
# }
# 
# eval <- read_feather("Data/Lichess Data/Processed Evals/evals_fixed_castling.feather", as_data_frame = FALSE)
# 
# eval <- eval %>% 
#   filter(!str_detect(pgn, "11\\."))
# 
# tic(msg = "Whole thing")
# tic(msg = "First stage eval")
# 
# eval <- eval %>%
#   dplyr::filter(!str_detect(pgn, "11\\.")) %>% 
#   dplyr::mutate(tmp = str_replace_all(pgn, "([0-9+]{0,}\\.)", ""),
...
# toc()
# 
# tic(msg = "Second stage eval")
# 
# eval <- bind_cols(eval, extract_moves(eval$moves, N = 10))
# 
# eval <- eval %>% 
#   select(W1:B10, numeric_eval)
# 
# write_feather(eval, "Data/Lichess Data/Processed Evals/evals_up_to_10_moves_second_stage.feather")
# 
# toc()

# eval <- read_feather("Data/Lichess Data/Processed Evals/evals_up_to_10_moves.feather")

df <- open_dataset("Data/Consolidated PGNs/", format = "feather") %>%
  dplyr::filter(time_control == "Rapid",
                month == "2022-01",
                rating_class == "2000-2200") %>%
  collect()
...
test <- df %>%
  ...
  ungroup() %>%
  mutate(b10_move_pct = b10_total_games/w10_total_games,
         ...
         b10_line_pct = b10_move_pct*b9_move_pct*b8_move_pct*b7_move_pct*b6_move_pct*b5_move_pct*b4_move_pct*b3_move_pct*b2_move_pct*b1_move_pct,
         w10_line_pct = w10_move_pct*w9_move_pct*w8_move_pct*w7_move_pct*w6_move_pct*w5_move_pct*w4_move_pct*w3_move_pct*w2_move_pct*w1_move_pct,
         b9_line_pct = b9_move_pct*b8_move_pct*b7_move_pct*b6_move_pct*b5_move_pct*b4_move_pct*b3_move_pct*b2_move_pct*b1_move_pct,
         w9_line_pct = w9_move_pct*w8_move_pct*w7_move_pct*w6_move_pct*w5_move_pct*w4_move_pct*w3_move_pct*w2_move_pct*w1_move_pct,
         b8_line_pct = b8_move_pct*b7_move_pct*b6_move_pct*b5_move_pct*b4_move_pct*b3_move_pct*b2_move_pct*b1_move_pct,
         w8_line_pct = w8_move_pct*w7_move_pct*w6_move_pct*w5_move_pct*w4_move_pct*w3_move_pct*w2_move_pct*w1_move_pct,
         b7_line_pct = b7_move_pct*b6_move_pct*b5_move_pct*b4_move_pct*b3_move_pct*b2_move_pct*b1_move_pct,
         w7_line_pct = w7_move_pct*w6_move_pct*w5_move_pct*w4_move_pct*w3_move_pct*w2_move_pct*w1_move_pct,
...
write_feather(test, "Data/Lichess Data/Processed Evals/test.feather")

test <- test %>% 
  filter(b10_total_games > 1)

test %>% 
  filter(w10_line_pct > 1/1000000000) %>% 
  select(W1:B10, w10_line_pct) %>% 
  arrange(desc(w10_line_pct))
...
test %>% 
  filter(b10_line_pct == max(b10_line_pct))
  # group_by(W1:B10) %>%
  # summarise(move_black_wins = sum(black_wins),
  #           move_white_wins = sum(white_wins),
  #           move_draws = sum(draws),
  #           move_total_games = sum(total_games)) %>% 
  # ungroup() %>% 
  # mutate(white_score = (move_white_wins + 3000)/(move_total_games + 10000),
  #        black_score = (move_black_wins + 3000)/(move_total_games + 10000)) %>% 
  # group_by(W1:W10) %>% 
  # filter(black_score == max(black_score)) %>% 
  # group_by(W1:B9) %>% 
  # mutate(move_black_wins = sum(move_black_wins),
  #        move_white_wins = sum(move_white_wins),
  #        move_draws = sum(move_draws),
  #        move_total_games = sum(move_total_games)) %>% 
  # ungroup() %>% 
  # mutate(white_score = (move_white_wins + 3000)/(move_total_games + 10000),
  #        black_score = (move_black_wins + 3000)/(move_total_games + 10000)) %>% 
  # group_by(W1:W9) %>% 
